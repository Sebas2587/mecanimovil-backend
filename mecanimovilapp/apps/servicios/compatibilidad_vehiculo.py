"""
Compatibilidad catálogo maestro Servicio ↔ vehículo (marca/modelo).
...
def queryset_servicios_catalogo_por_marca(marca_id) -> QuerySet[Servicio]:
    """
    ...
    if marca_key == '0':
        return queryset_servicios_genericos().order_by('nombre')
...
def queryset_servicios_catalogo_por_marca_modelo(
    ...
    """Servicios del catálogo maestro compatibles con marca/modelo del vehículo."""
    if not marca:
        return Servicio.objects.none()

    base = queryset_servicios_catalogo_por_marca(marca.id)
...
def queryset_servicios_compatibles_vehiculo(vehiculo: Vehiculo | None) -> QuerySet[Servicio]:
    ...
    return queryset_servicios_catalogo_por_marca_modelo(modelo, marca)
...
def servicios_comunes_por_marcas_queryset(marca_ids: list[int]) -> QuerySet[Servicio]:
    """Intersección de servicios compatibles con todas las marcas indicadas."""
    ids = [int(m) for m in marca_ids if str(m) not in ('', '0')]
    if not ids:
        return Servicio.objects.none()
    if len(ids) == 1:
        return queryset_servicios_catalogo_por_marca(ids[0])

    conjuntos = [
        set(queryset_servicios_catalogo_por_marca(marca_id).values_list('id', flat=True))
        for marca_id in ids
    ]
